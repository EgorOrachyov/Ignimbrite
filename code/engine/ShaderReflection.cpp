...
Shader::DataType getDimType(Shader::DataType baseType, uint32 rows, uint32 columns) {
    ...
    switch (baseType) {
        case Shader::DataType::Bool:
            if (columns == 1) {
                switch (rows) {
                    case 2: return Shader::DataType::Bool2;
                    case 3: return Shader::DataType::Bool3;
                    case 4: return Shader::DataType::Bool4;
                    default: ;
                }
            } else {
        ...
        case Shader::DataType::Int:
            if (columns == 1) {
                switch (rows) {
                    case 2: return Shader::DataType::Int2;
                    case 3: return Shader::DataType::Int3;
                    case 4: return Shader::DataType::Int4;
                    default: ;
                }
            } else {
        ...
        case Shader::DataType::UInt:
            if (columns == 1) {
                switch (rows) {
                    case 2: return Shader::DataType::UInt2;
                    case 3: return Shader::DataType::UInt3;
                    case 4: return Shader::DataType::UInt4;
                    default: ;
                }
            } else {
        ...
        case Shader::DataType::Float:
            if (columns == 1) {
                switch (rows) {
                    case 2: return Shader::DataType::Float2;
                    case 3: return Shader::DataType::Float3;
                    case 4: return Shader::DataType::Float4;
                    default: ;
                }
            } else {
                if (columns == rows) {
                    switch (columns) {
                        case 2: return Shader::DataType::Mat2;
                        case 3: return Shader::DataType::Mat3;
                        case 4: return Shader::DataType::Mat4;
                        default: ;
                    }
                } else {
...
Shader::DataType getType(const spirv_cross::Compiler &comp,
    ...
    switch (type.basetype) {
        ...
        default:
            throw std::runtime_error("Base type of this var is not supported: " + comp.get_name(type.self) + " " + memberName);
    }
...
void getSpirvParams(
    ...
    for (const auto &resource : resources.uniform_buffers) {
        ...
        parseSpirvStruct(params, uniformInfo.members, stageFlags, comp, spirType, resource.name, uniformInfo.binding, 0);
    }
...
void ShaderReflection::reflect(Shader& shader)
{
    for (const auto &desc : shader.mProgramDesc.shaders) {
        ...
        if (shader.mProgramDesc.language == ShaderLanguage::SPIRV) {
            ...
            spirv_cross::Compiler comp((const uint32_t*)desc.source.data(), wordCount);
            const spirv_cross::ShaderResources &resources = comp.get_shader_resources();

            getSpirvParams(comp, resources, shader.mVariables, shader.mBuffers, stageFlags);

            if (desc.type == ShaderType::Vertex) {
                getSpirvModuleInputs(comp, resources, shader.mVertexShaderInputs);
            } else if (desc.type == ShaderType::Fragment) {
                getSpirvModuleOutputs(comp, resources, shader.mFragmentShaderOutputs);
            }
